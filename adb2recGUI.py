...
def install_requirements():
	...
	command = "sudo apt-get install adb figlet cowsay ruby-notify flite feh"
...
def adb_connect_ip_wifi():
	...
	#subprocess.Popen(command, shell=True)
	#time.sleep(3.0)
	rawip = raw_input('Insert private IP of your Mobile Phone, like this \'192.168.43.1\' : ')
	ip = str(rawip)
	try:
		socket.inet_aton(ip)
		print("Valid IP")
...
def video2ocrcolor():
	os.getcwd()
	langraw = raw_input('Insert language, example eng ita ara : ')
	lang = str(langraw)
...
label = tk.Label(text=".          Mobile Forensic Androis                ",
				fg="darkblue",
				font=("helvetica",10),
				
					)
label.pack(ipadx=15, ipady=4, pady=10, padx=25)
label.place(x=0, y=0)
#ok
button = tk.Button(frame, 
                   ...
                   bg="#ff0000",
                   command=quit)
button.pack(ipadx=128, ipady=4, pady=20)
