...
def compressandhash():
    ...
    if os.path.exists(screenshot_dir):
        screenshot_archive = os.path.join(compress_dir, 'screenshot')
        archive_file = shutil.make_archive(screenshot_archive, 'gztar', root_dir=screenshot_dir)
        archive_paths['screenshot'] = archive_file
        print(f"✅ screenshot compresso in {archive_file}")
    else:
        print("⚠️  Cartella screenshot non trovata")

    # 📦 Comprime screenrecord
    if os.path.exists(screenrecord_dir):
        screenrecord_archive = os.path.join(compress_dir, 'screenrecord')
        archive_file = shutil.make_archive(screenrecord_archive, 'gztar', root_dir=screenrecord_dir)
        archive_paths['screenrecord'] = archive_file
        print(f"✅ screenrecord compresso in {archive_file}")
    else:
        print("⚠️  Cartella screenrecord non trovata")

    # 🔐 Calcola hash per ogni archivio
    for name, archive_path in archive_paths.items():
        hash_txt = os.path.join(hash_dir, f'hash_{name}.txt')
        ...
        with open(hash_txt, 'w') as f:
            f.write(f"MD5: {md5}\nSHA256: {sha}\n")
        print(f"🔐 Hash salvati in {hash_txt}")

    # 📱 Recupera ID Android
    try:
        result = subprocess.run(['adb', 'shell', 'settings', 'get', 'secure', 'android_id'],
                                capture_output=True, text=True, check=True)
        android_id = result.stdout.strip()
    except Exception as e:
        print(f"⚠️  Errore nel recuperare ANDROID_ID: {e}")
        android_id = "UNKNOWN"

    # 📂 Crea directory finale con ID e timestamp
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    final_dir = os.path.join(base_dir, f"{android_id}-{timestamp}")
    os.makedirs(final_dir, exist_ok=True)

    # 📦 Sposta cartelle originali e archiviate
    for path in [screenshot_dir, screenrecord_dir, compress_dir, hash_dir]:
        if os.path.exists(path):
            shutil.move(path, final_dir)

    # 🧹 Sposta eventuali file temporanei
